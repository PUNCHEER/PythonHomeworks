...
def delete_contact(new: dict, index: int):
    global phone_book
    name = ''
    for contatc in phone_book:
        if index == contatc.get('id'):
            name = contatc.get('name')
            phone_book.remove(contatc)
    for contatc in range(0, len(phone_book)):
        id = contatc + 1
        contact_id = phone_book[contatc]
        contact_id['id'] = str(id)
    return name
